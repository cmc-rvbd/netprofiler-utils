...
#
# This script isa slightly augmented version of the standard example script allowing
# a "limit" parameter to be specified to override the default limit of 10K rows in a
# report table. Usage "--limit x" where "x" should be the new limit. Note that this
# limit has to be applied twice in the code - once when running the report and once
# when getting the data. Use values higher than 10K WITH CAUTION - this default limit
# protects netProfiler from being overloaded with generating excessively long reports.
#
...
class TrafficSummaryApp(NetProfilerApp):
    ...
    def add_options(self, parser):
        ...
        group.add_option('--limit', dest='limit', default=None,
                         help='Limit the number of rows (default 10K)')
    ...
    def main(self):
        ...
        with TrafficSummaryReport(self.netprofiler) as report:
            report.run(columns=self.options.columns.split(','),
                       ...
                       resolution='auto', limit=self.limit)
            data = report.get_data(limit=self.limit)
